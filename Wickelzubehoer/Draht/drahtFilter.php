...
$filterResult = get_drahtFilter($conn);
$markeArray = array();
$preisArray = array();
...
while($row = mysqli_fetch_array($filterResult))
{
    ...
    if(!in_array($row['durchmesser'], $durchmesser)){
        array_push($durchmesser, $row['durchmesser']);
    }
    if(!in_array($row['laenge'], $laenge)){
        array_push($laenge, $row['laenge']);
    }
    if(!in_array($row['material'], $material)){
...
<div class="container-fluid" style="padding: 0; width: 100%; margin-left: 0;margin-bottom: 10px; background-color: whitesmoke">
    ...
    <div style="margin: 10px">
        ...
        for($i = 0; $i < count($material); $i++){
            ?>
            <div class="checkbox">
                <label><input type="checkbox" value="<?php echo $material[$i]; ?>"><?php echo $material[$i]; ?> </label>
            </div>
            <?php
        }
        ?>
        ...
        <?php
        global $durchmesser;
        for($i = 0; $i < count($durchmesser); $i++){
            ?>
            <div class="checkbox">
                <label><input type="checkbox" value="<?php echo $durchmesser[$i]; ?>"><?php echo $durchmesser[$i]; ?> ga</label>
            </div>
            <?php
        }
        ?>
        ...
        <?php
        global $laenge;
        for($i = 0; $i < count($laenge); $i++){
            ?>
            <div class="checkbox">
                <label><input type="checkbox" value="<?php echo $laenge[$i]; ?>"><?php echo $laenge[$i]; ?> m</label>
            </div>
            <?php
        }
        ?>
