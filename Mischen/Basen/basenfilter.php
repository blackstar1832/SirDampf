...
$filterResult = get_basenFilter($conn);
$markeArray = array();
$preisArray = array();
$bewertungArary = array();
$groesseArray = array();
$basenVerhaeltnis = array();
while($row = mysqli_fetch_array($filterResult))
{
    ...
    if(!in_array($row['basenVerhaeltnis'], $basenVerhaeltnis)){
        array_push($basenVerhaeltnis, $row['basenVerhaeltnis']);
    }
}
?>
<div class="container-fluid" style="padding: 0; width: 100%; margin-left: 0;margin-bottom: 10px; background-color: whitesmoke">
    ...
    <div style="margin: 10px">
        ...
        <h4>Mischverhältniss</h4>
        <?php
        global $basenVerhaeltnis;
        for($i = 0; $i < count($basenVerhaeltnis); $i++){
            ?>
            <div class="checkbox">
                <label><input type="checkbox" value="<?php echo $basenVerhaeltnis[$i]; ?>"><?php echo $basenVerhaeltnis[$i]; ?></label>
            </div>
            <?php
        }
        ?>
